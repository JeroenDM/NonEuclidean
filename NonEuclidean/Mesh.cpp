...
Mesh::Mesh(const char* fname) {
    ...
    if (line.find("v ") == 0) {
      ...
      for (size_t i = 0; i < line.size(); ++i) {
        if (line[i] == '/') {
          ...
          if (last_slash_ix == i - 1) {
            assert(vert_palette.size() == uv_palette.size() ||
                   uv_palette.empty());
            doubleslash = true;
          }
          last_slash_ix = i;
      ...
      if (wild) {
        ...
        if (doubleslash) {
          ...
          ss >> a >> at >> _tmp >> b >> bt >> _tmp >> c >> ct >> _tmp >> d >>
              dt >> _tmp;
        }
      } else {
        assert(false);
  ...
  }
  {
    ...
    glBufferData(GL_ARRAY_BUFFER, uvs.size() * sizeof(uvs[0]), uvs.data(),
                 GL_STATIC_DRAW);
...
void Mesh::AddFace(const std::vector<float>& vert_palette,
                   const std::vector<float>& uv_palette, uint32_t a,
                   uint32_t at, uint32_t b, uint32_t bt, uint32_t c,
                   uint32_t ct, bool is3DTex) {
