...
class Vector3 {
 public:
  ...
  inline float Mag() const { return std::sqrt(MagSq()); }

  // Normalization
  inline void Normalize() { (*this) /= Mag(); }
  inline void NormalizeSafe() { (*this) /= (Mag() + FLT_EPSILON); }
  inline Vector3 Normalized() const { return (*this) / Mag(); }
...
class Matrix4 {
 public:
  ...
  // General
  inline void Fill(float b) { std::fill(m, m + 16, b); }
  inline void MakeZero() { Fill(0.0f); }
  ...
  // Some getters
  inline Vector3 XAxis() const { return Vector3(m[0], m[4], m[8]); }
  inline Vector3 YAxis() const { return Vector3(m[1], m[5], m[9]); }
  inline Vector3 ZAxis() const { return Vector3(m[2], m[6], m[10]); }
  inline Vector3 Translation() const { return Vector3(m[3], m[7], m[11]); }
  inline Vector3 Scale() const { return Vector3(m[0], m[5], m[10]); }
  ...
  inline void operator/=(float b) { operator*=(1.0f / b); }
  ...
  Matrix4 operator*(const Matrix4& b) const {
    ...
    out.m[10] =
        b.m[2] * m[8] + b.m[6] * m[9] + b.m[10] * m[10] + b.m[14] * m[11];
    out.m[11] =
        b.m[3] * m[8] + b.m[7] * m[9] + b.m[11] * m[10] + b.m[15] * m[11];

    out.m[12] =
        b.m[0] * m[12] + b.m[4] * m[13] + b.m[8] * m[14] + b.m[12] * m[15];
    out.m[13] =
        b.m[1] * m[12] + b.m[5] * m[13] + b.m[9] * m[14] + b.m[13] * m[15];
    out.m[14] =
        b.m[2] * m[12] + b.m[6] * m[13] + b.m[10] * m[14] + b.m[14] * m[15];
    out.m[15] =
        b.m[3] * m[12] + b.m[7] * m[13] + b.m[11] * m[14] + b.m[15] * m[15];
    return out;
  }
  void operator*=(const Matrix4& b) { (*this) = operator*(b); }
  Vector4 operator*(const Vector4& b) const {
    return Vector4(m[0] * b.x + m[1] * b.y + m[2] * b.z + m[3] * b.w,
                   m[4] * b.x + m[5] * b.y + m[6] * b.z + m[7] * b.w,
                   m[8] * b.x + m[9] * b.y + m[10] * b.z + m[11] * b.w,
                   m[12] * b.x + m[13] * b.y + m[14] * b.z + m[15] * b.w);
  }
  Vector3 MulPoint(const Vector3& b) const {
    const Vector3 p(m[0] * b.x + m[1] * b.y + m[2] * b.z + m[3],
  ...
  Vector3 MulDirection(const Vector3& b) const {
    return Vector3(m[0] * b.x + m[1] * b.y + m[2] * b.z,
  ...
  Matrix4 Inverse() const {
    ...
    const float det =
        m[0] * inv.m[0] + m[1] * inv.m[4] + m[2] * inv.m[8] + m[3] * inv.m[12];
    inv /= det;
    return inv;
  }
