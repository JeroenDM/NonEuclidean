...
Texture::Texture(const char* fname, int rows, int cols) {
  ...
  // Open the bitmap
  std::ifstream fin(std::string("Textures/") + fname,
  ...
  for (int y = height; y-- > 0;) {
    ...
    for (int x = 0; x < width; x++) {
      ...
      uint8_t* ptr =
          img +
          ((row * cols + col) * (block_w * block_h) + ty * block_w + tx) * 3;
  ...
  if (is3D) {
    ...
    glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RGB8, width / rows, height / cols,
                 rows * cols, 0, GL_BGR, GL_UNSIGNED_BYTE, img);
  } else {
